    ...
    <div className="space-y-4 pb-24">
      ...
      <div className="fixed bottom-16 left-0 right-0 bg-background/95 backdrop-blur-sm border-t p-4 z-40">
        <div className="flex items-center gap-3">
          ...
          {/* CTA Primaria: Completa o Consuntiva (solo se NON presa in carico) */}
          {!showPresaInCarico && canBeCompleted && !canRequestSignature && (
            <MobileButton
              variant="default"
              className="flex-1"
              onClick={onCompleta}
          ...
          {!showPresaInCarico && canBeConsuntivato && (
            <MobileButton
              variant="default"
              className="flex-1"
          ...
          {!showPresaInCarico && servizio.stato === 'da_assegnare' && isAdmin && (
            <MobileButton variant="default" className="flex-1" onClick={onAssegna}>
              <User className="h-5 w-5" />
              Assegna Servizio
            </MobileButton>
          )}
          {!showPresaInCarico && servizio.stato !== 'da_assegnare' && !canBeCompleted && !canBeConsuntivato && (canBeEdited || isAdmin) && (
            <div className="flex-1" />
          )}
